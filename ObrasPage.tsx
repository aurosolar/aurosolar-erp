...
const ESTADOS_CONFIG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  ...
  VALIDACION_OPERATIVA: { label: 'Validación', color: 'text-estado-purple', bg: 'bg-estado-purple/10', dot: 'bg-estado-purple' },
  REVISION_COORDINADOR: { label: 'Rev. Coordinador', color: 'text-estado-purple', bg: 'bg-estado-purple/10', dot: 'bg-estado-purple' },
...
export default function ObrasPage() {
  ...
  const cargarObras = useCallback(async () => {
    ...
    // For kanban, load more to see all columns
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        ...
        <div className="flex items-center gap-3">
          {/* View toggle */}
          <div className="flex bg-auro-surface-2 rounded-lg p-0.5">
            <button
              onClick={() => setVista('kanban')}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                vista === 'kanban'
                  ? 'bg-white text-auro-navy shadow-sm'
                  : 'text-auro-navy/40 hover:text-auro-navy/60'
              }`}
              title="Vista Kanban"
            >
              ▦ Kanban
      ...
      {/* Contadores por estado (solo en vista tabla) */}
      {vista === 'tabla' && (
      ...
      {loading ? (
        ...
        <KanbanBoard
          obras={obras}
          onObraClick={(id) => setObraSeleccionada(id)}
